function [coverage,Covered_Area] = Cov_Func_RD_v1(pop,rs,theta0,Obstacle_Area,Covered_Area)
...
% count covered points on wanted location  (wanted)
count1=sum(sum(Covered_Area));		                           
% count covered points on unwanted location (obstacles)
count2=numel(find(Covered_Area == -2));		                           
% count total points on wanted location
count3=sum(sum(Obstacle_Area));           
% function to avoid obstacles
coverage=((count1-count2)/count3);	                            
% function to aim on wanted area
%coverage=(count1/count3);		                                
