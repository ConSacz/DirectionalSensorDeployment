...
for it = 1:MaxIt
    for i=1:nPop
        for j=1:N
            ...
            k=randi([1,N]);
            alpop_v=w*pop(i).Velocity(j,1:3) + c1*rand()*(pop(i).Position(k,1:3) - pop(i).Position(j,1:3)) + c2*rand()*(BestSol.Position(j,1:3) - pop(i).Position(j,1:3));
            alpop(j,1:3) = pop(i).Position(j,1:3) + alpop_v;
            ...
            if Connectivity_graph(Graph(alpop(:,1:2),rc),[]) == 1
                ...
                if alpop_cov > pop(i).Cost
                    ...
                    pop(i).Velocity(j,1:3) = alpop_v;
                end
            end
        end
        pop(i).Velocity(j,1:3) = pop(i).Velocity(j,1:3) * wdamp;
    end
